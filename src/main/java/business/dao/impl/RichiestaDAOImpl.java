    ...
    public int saveRichiesta(Richiesta richiesta) throws DataLayerException {
        ...
        try (Connection connection = Database.getDatasource().getConnection()) {
            try (PreparedStatement ps = connection.prepareStatement(SAVE_RICHIESTA)) {
                ...
                ps.setString(3, richiesta.getDocente().getNome());
                ps.setString(4, richiesta.getDocente().getCognome());
            ...
            try (PreparedStatement ps = connection.prepareStatement(GET_RICHIESTA_STUDENTE)) {
                ...
                try (ResultSet rset = ps.executeQuery()) {
                    ...
                    if (rset.next()) {
                        ...
                        richiesta.setDataInizio(rset.getDate("dataInizio"));
                        richiesta.setDataFine(rset.getDate("dataFine"));
                        richiesta.setCrediti(rset.getInt("crediti"));
                        richiesta.setDocente(new Docente(rset.getString("nomeDocente"),rset.getString("cognomeDocente")));
                    }
                }
            }
        } catch (SQLException ex) {
